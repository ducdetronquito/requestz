...
pub fn Connection(comptime SocketType: type) type {
    return struct {
        ...
        pub fn stream(self: *Self, method: Method, uri: Uri, options: anytype) !ResponseStream(Self) {
            ...
            var buffer: [4096]u8 = undefined;
            var event = try self.state.nextEvent(&buffer);
            const status_code = event.Response.status_code;
            const version = event.Response.version;

            const allocator = arena.allocator();
            var headers = Headers.init(allocator);
            while (true) {
                event = try self.state.nextEvent(&buffer);
                switch (event) {
                    .Header => |*header| {
                        header.name.value = try allocator.dupe(u8, header.name.raw());
                        header.value = try allocator.dupe(u8, header.value);
...
test "Get" {
    ...
    try expectEqualStrings(response.headers[0].name.raw(), "Content-Length");
    try expectEqualStrings(response.headers[0].value, "14");
    try expectEqualStrings(response.headers[1].name.raw(), "Server");
    try expectEqualStrings(response.headers[1].value, "gunicorn/19.9.0");
...
test "Get a streaming response" {
    ...
    try expectEqualStrings(response.headers[0].name.raw(), "Content-Length");
    try expectEqualStrings(response.headers[0].value, "12288");
...
test "Get a chunk encoded response" {
    ...
    try expectEqualStrings(response.headers[0].name.raw(), "Transfer-Encoding");
    try expectEqualStrings(response.headers[0].value, "chunked");
